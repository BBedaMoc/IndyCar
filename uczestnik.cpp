...
Uczestnik::Uczestnik(float modS, float modP, float modH){
    modHamowania=modH;
...
Uczestnik::Uczestnik(const Uczestnik& el){
...
void Uczestnik::setNrAuta(int i){
    nrAuta=i;
}
void Uczestnik::setLapToGo(int i){
    lapToGo=i;
}

void Uczestnik::setAktywnaTx(int i){
    aktywnaTx=i;
}

void Uczestnik::setPozycja(int i){
    pozycja=i;
}

int Uczestnik::getLapToGo(){
    return lapToGo;
}

int Uczestnik::getAktywnaTx(){
    return aktywnaTx;
}

int Uczestnik::getPozycja(){
    return pozycja;
}
void Uczestnik::setIter(float i){
    iter=i;
}

void Uczestnik::setNrEt(int nr){
    nrEt=nr;
}

float Uczestnik::getIter(){
    return iter;
}

int Uczestnik::getNrEt(){
    return nrEt;
}

void Uczestnik::setKoniecWys(bool w){
    koniecWys=w;
}
void Uczestnik::inicjalizujMojeBT(Uczestnik* ref){
...
void Uczestnik::inicjalizujMojeBT(std::vector <Obstacle*> T, std::vector <Obstacle*> B){
    std::vector <Obstacle*> pomT, pomB;
    std::vector <bool> kolT, kolB;
...
void Uczestnik::aktPosElToru(float angle, float poX, float poY, Point srObr, bool zak){
    if(!zak)
    {
        for (int i=0; i<int(mojeB.size());i++){
...
float Uczestnik::minXY(std::vector <Obstacle*> el, float wspX, bool czyX){
    float min;
    if(czyX){
        min=el[0]->getPosX();
...
std::vector <Obstacle *> Uczestnik::getZakret(int dl, std::string rdz, Point pocz, int dokl){
